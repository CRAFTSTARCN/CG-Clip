...
void PolygenController::onActivate() {
    Component* clipper = attachedObject->getComponent("Clipper");
    if(!clipper) throw Throwable("Can not found clipper component of owner object");
    pointList = dynamic_cast<Clipper*>(clipper)->getPointLoop();
